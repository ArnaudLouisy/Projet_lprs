                            ...
                            <?php foreach ($offrevalider as $value): ?>
                                <div class='single-job-items mb-30'>
                                    ...
                                    <div class='items-link items-link2 f-right'>
                                        <a type='submit'
                                           href='job_details.php?id=<?= $value['id_offre']?>'><?= $value['type_contrat'] ?></a>
                                        <span> <?php $date = explode(' ', $value['date_publication']);
                                            $jourj = date_create("now");
                                            $jourp = date_create($date[0]);
                                            $intervale = date_diff($jourp, $jourj);

                                            if ($intervale->i <= 59 && $intervale->d <= 31 && $intervale->m <= 0 && $intervale->y <= 0) {
                                                echo("Il y a ".$intervale->format('%R%d')." Jour(s)");
                                            }elseif ($intervale->i <= 59 && $intervale->d <= 31  && $intervale->m <= 12 && $intervale->y <= 0) {
                                                echo("Il y a ".$intervale->format('%R%m')." Moi(s)");
                                            }elseif ($intervale->i >= 0 && $intervale->d <= 0  && $intervale->m <= 0 && $intervale->y <= 0) {
                                                echo("Il y a ".$intervale->format('%R%i')." Minute(s)");
                                            }?> </span>
                                    </div>
                                </div>
                            <?php endforeach; ?>
